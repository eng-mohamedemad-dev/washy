...
class _LoginPageState extends State<LoginPage> {
  ...
  void initState() {
    ...
    context.read<LoginBloc>().add(const LoginPhoneNumberChanged(phoneNumber: ''));
  }
        ...
        listener: (context, state) {
          if (state is LoginLoading) {
            ...
            if (state is LoginError) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Welcome back ${state.user.name ?? 'User'}!')),
              );
            }
          }
        },
  ...
  Widget _buildContent(LoginState state) {
    ...
    return Column(
      ...
      children: [
        ...
        // Welcome text (like Java's login description)
        const Text(
          'Welcome Back!',
          style: TextStyle(
            fontSize: 24,
        ...
        // OR Divider (like Java's divider)
        const Row(
          children: [
        ...
        // Social Login Section (like Java's social login for login)
        SocialLoginSection(
          onGoogleLogin: () {
            context.read<LoginBloc>().add(LoginGoogleSignInPressed());
          },
          onEmailLogin: () {
